...
function assertReadOnlyQuery(query: string): void {
	if (/\bmutation\b/i.test(query)) {
		throw new Error(
			"Blocked: GraphQL mutations are not permitted. Read-only queries only.",
		);
	}
}
...
async function getRepoOwner(
	...
	// Host-agnostic: handles SSH aliases (github.com-alias), HTTPS, ssh://, git://
...
async function execGraphQL(query: string): Promise<unknown> {
	...
	if (!stdout.trim()) {
		throw new Error(
			`gh api graphql returned empty response. stderr: ${stderr}`,
		);
	}
...
export const getRepoPullRequestsPage = cache(
	async (
		...
		return {
			prs: page.nodes
				.map(transformNode)
				.map((pr) => ({ ...pr, metrics: computePrMetrics(pr) })),
